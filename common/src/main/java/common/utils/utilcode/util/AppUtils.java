...
public final class AppUtils {
    ...
    public static List<String> getInstallApks(Context context, boolean ignoreSystem) {
        List<String> pkgs = new ArrayList<>();
        List<PackageInfo> packageInfos = context.getPackageManager().getInstalledPackages(0);
        for (PackageInfo info : packageInfos) {
            //过滤掉系统app
            if (ignoreSystem && (ApplicationInfo.FLAG_SYSTEM & info.applicationInfo.flags) != 0) {
                continue;
            }
            //过滤掉google应用
            if (ignoreSystem && info.packageName.contains("google")) {
                continue;
            }
